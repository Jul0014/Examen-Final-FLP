...
import (  
  ...
  "path/filepath"; "fmt"; "strings" ;"sync"
)
...
func main() { 
  ...
  wg := new(sync.WaitGroup)//1. creamos un wait group 
  
  start := time.Now()

  for y := 0; y < size.Y; y++ { 
    wg.Add(1)//6. agregar un contador
    y:= y
    //7. luego llamo a la func. g rutine que es la concurrente 
    go func(){
      for x := 0; x < size.X; x++ { 
